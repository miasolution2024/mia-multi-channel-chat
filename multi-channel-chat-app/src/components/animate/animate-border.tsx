...
export function AnimateBorder({ animate, sx }: any) {
  ...
  const background = (color: any) => {
    ...
    const end = `transparent ${
      values.angle - (2 + values.length)
    }deg, ${color}  ${values.angle}deg, transparent ${
    ...
    return [
      ...
      `conic-gradient(from ${degs[1]}deg at ${
        100 - values.distance / aspectRatio
      }% ${values.distance}% , ${end})`,
      `conic-gradient(from ${degs[2]}deg at ${
        100 - values.distance / aspectRatio
      }% ${100 - values.distance}% , ${end})`,

...
import { ContentSuggestionItem } from "@/sections/marketing-campaign/types/create-post-list-step";
import {
  Box,
  Button,
  Chip,
...
} from "@/sections/marketing-campaign/utils";
...
export function CreatePostListStep() {
  ...
  >([]);
  const [isLoadingContentAssistant, setIsLoadingContentAssistant] =
    useState(false);
  const [deleteConfirmOpen, setDeleteConfirmOpen] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<string | null>(null);
  const [editDrawerOpen, setEditDrawerOpen] = useState(false);
  const [itemToEdit, setItemToEdit] = useState<ContentSuggestionItem | null>(
    null
  );
  const [createPostListDialogOpen, setCreatePostListDialogOpen] =
    useState(false);
  const [createAdditionalPost, setCreateAdditionalPost] = useState(false);
  const [additionalPostQuantity, setAdditionalPostQuantity] =
    useState<number>(0);
  ...
  useEffect(() => {
    const loadContentAssistants = async () => {
      if (
        ...
        !Array.isArray(aiContentSuggestions) ||
    ...
    editForm.reset({
      ...
      content_tone: item.content_tone
        .map((ct) => {
          ...
          const toneOption = contentToneOptions.find(
            (option) =>
      ...
      if (suggestion.id === itemToEdit.id) {
        return {
          ...
          secondary_seo_keywords: Array.isArray(formData.secondary_seo_keywords)
            ...
            : (formData.secondary_seo_keywords as string)
                .split(",")
                .map((k: string) => k.trim()),
  ...
  const handlePostsConfirm = (selectedItems: Content[]) => {
    ...
    const trulyNewItems = selectedItems.filter(
      (item) => !currentIds.has(item.id.toString())
    );

    const remainingItems = contentSuggestions;

    const newContentSuggestions: ContentSuggestionItem[] = trulyNewItems.map(
      (item) => ({
        id: item.id.toString(),
        topic: item.topic,
        main_seo_keyword: item.main_seo_keyword,
    ...
    const updatedContentSuggestions = [
      ...remainingItems,
    ...
    setValue(
      ...
      updatedContentSuggestions.map((item) => item.id)
    );
  ...
  const handleGenerateAdditionalPosts = async () => {
    ...
    try {
      ...
      const contentAssistantFormData = transformCampaignToContentAssistant(
        formData as CampaignFormData
      );

      // Build research data
      const researchData = await buildStepResearchData(
        contentAssistantFormData,
        true
      );
      // Create multiple content assistants using Promise.allSettled
      const createPromises = Array.from(
        { length: additionalPostQuantity },
        () =>
      ...
      if (failedCount > 0) {
        console.warn(
          `${failedCount} out of ${results.length} posts failed to create`
        );
      }
      ...
      const newContentSuggestions = detailedResults.filter(
        (item) => item !== null
      ) as ContentSuggestionItem[];
      ...
      toast.success(
        `Đã tạo thành công ${newContentSuggestions.length} bài viết`
      );
    ...
    },
    {
      ...
      render: (item) => {
        ...
        return (
          ...
          >
            {suggestion.customer_journey
              .map((cj) => cj.customer_journey_id.name)
              .join(", ")}
          </Typography>
        );
      },
        ...
        return (
          ...
          >
            {POST_TYPE_OPTIONS.find(
              (option) => option.value === suggestion.post_type
            )?.label || suggestion.post_type}
          </Typography>
        );
      },
    },
    {
      ...
      render: (item) => {
        ...
        return (
          ...
          >
            {suggestion.content_tone
              .map((ct) => ct.content_tone_id?.tone_description)
              .map((item) => (
      ...
      >
        ...
        <DialogContent>
          ...
          <TextField
            ...
            onChange={(e) =>
              setAdditionalPostQuantity(parseInt(e.target.value))
            }
          />
        </DialogContent>
        <DialogActions>
          ...
          <Button
            ...
            disabled={isGeneratingPosts}
            startIcon={
              isGeneratingPosts ? <CircularProgress size={16} /> : null
            }
          >

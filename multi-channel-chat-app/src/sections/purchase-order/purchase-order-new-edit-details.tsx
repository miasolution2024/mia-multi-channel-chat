...
export function PurchaseOrderNewEditDetails() {
  ...
  const totalOnRow = values.purchaseOrderDetails.map(
  ...
  const handleClearService = useCallback(
    (index: number) => {
      setValue(`purchaseOrderDetails[${index}].quantity`, 1);
      setValue(`purchaseOrderDetails[${index}].purchasePrice`, 0);
      setValue(`purchaseOrderDetails[${index}].subTotal`, 0);
    },
    [setValue]
  );
  ...
  const handleSelectProduct = useCallback(
    (index: number, product: Product) => {
      setValue(`purchaseOrderDetails[${index}].productID`, product.productID);
    },
    [setValue]
  );
  ...
  const handleChangeQuantity = useCallback(
    (event: any, index: number) => {
      setValue(
        `purchaseOrderDetails[${index}].quantity`,
        Number(event.target.value)
      );
      setValue(
        `purchaseOrderDetails[${index}].subtotal`,
        values.purchaseOrderDetails.map(
          (item: PurchaseOrderDetail) => item.quantity * item.purchasePrice
        )[index]
      );
    },
    [setValue, values.purchaseOrderDetails]
  );

  const handleChangePrice = useCallback(
    (event: any, index: number) => {
      setValue(
        `purchaseOrderDetails[${index}].purchasePrice`,
        Number(event.target.value)
      );
      setValue(
        `purchaseOrderDetails[${index}].subtotal`,
  ...
  const handleTaxesChange = useCallback(
    (event: any) => {
      ...
      console.log(values.purchaseOrderDetails);

      values.purchaseOrderDetails.forEach(
      ...
      >
        {fields.map((item, index) => (
          <Stack key={item.id} alignItems="flex-end" spacing={1.5}>
            ...
            >
              <Field.Select
                name={`purchaseOrderDetails[${index}].productID`}
                size="small"
                label="Products"
              ...
              <Field.Text
                size="small"
                name={`purchaseOrderDetails[${index}].unit`}
                label="Unit"
                InputLabelProps={{ shrink: true }}
              />
              ...
              <Field.Text
                size="small"
                type="number"
                name={`purchaseOrderDetails[${index}].quantity`}
                label="Quantity"
                placeholder="0"
              ...
              <Field.Text
                size="small"
                type="number"
                name={`purchaseOrderDetails[${index}].purchasePrice`}
                label="Price"
                placeholder="0.00"
              ...
              <Field.Text
                ...
                size="small"
                type="number"
                name={`purchaseOrderDetails[${index}].subtotal`}
                label="Total"
                placeholder="0.00"
                InputProps={{
                  startAdornment: (

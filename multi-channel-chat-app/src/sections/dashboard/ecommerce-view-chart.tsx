...
import { useStatisticViewData } from "./hooks/statistic-view-data";
import { OmniChoices } from "./type";

interface EcommerceViewChartProps {
  pages: OmniChoices[];
  startDate: string;
  endDate: string;
}

const EcommerceViewChart: React.FC<EcommerceViewChartProps> = ({
  pages,
  startDate,
  endDate,
}) => {
  ...
  const { processedData, isLoading, error, hasEnoughData } =
    useStatisticViewData({
      pages,
      method: "page_views_total",
      startDate,
      endDate,
        ...
        >
          ...
          >
            {!pages || pages.length === 0
              ...
              : "Không có dữ liệu lượt xem trong khoảng thời gian đã chọn."}
          </Typography>
        </Box>
      </Card>
    );
...
export default EcommerceViewChart;

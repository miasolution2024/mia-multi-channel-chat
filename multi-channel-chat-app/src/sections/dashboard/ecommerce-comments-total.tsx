...
interface EcommerceCommentsTotalProps {
  pages: OmniChoices[];
  startDate: string;
  endDate: string;
}

const EcommerceCommentsTotal: React.FC<EcommerceCommentsTotalProps> = ({
  pages,
  startDate,
  endDate,
}) => {
  const [commentsTotal, setCommentsTotal] = useState<DashboardTotalData[]>([]);
    ...
    if (Array.isArray(dashboardData?.data)) {
      const dashboardTotalData: DashboardTotalData[] = dashboardData.data.map(
        (item: {
          ...
          prev_comments_count?: number;
        }) => ({
          id: item.id,
          presentData: item.comments_count ?? 0,
          previosData: item.prev_comments_count ?? 0,
        })
      );
      setCommentsTotal(dashboardTotalData);
    } else {
      setCommentsTotal([]);
    }
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <Iconify icon="bxs:comment" style={{ color: "#F0B94B" }} />
            </Box>
...
export default EcommerceCommentsTotal;

...
var esSetTostringtag = function setToStringTag(object, value) {
	...
	if (toStringTag$1 && (overrideIfSet || !hasOwn$1(object, toStringTag$1))) {
		if ($defineProperty) {
			...
			object[toStringTag$1] = value; // eslint-disable-line no-param-reassign
		}
	}
};
...
function redirectToFrontend(res, frontendBaseUrl) {
  const redirectUrl = `${frontendBaseUrl}/admin/content/omni_channels`;
  res.redirect(redirectUrl);
}
function redirectToErrorPage(res, frontendBaseUrl, logId) {
  const redirectUrl = logId ? `${frontendBaseUrl}/admin/content/integration_logs/${logId}` : `${frontendBaseUrl}/admin/content/integration_logs`;
  res.redirect(redirectUrl);
}
function generatePKCE() {
  const code_verifier = crypto$1.randomBytes(32).toString("base64url");
  const hash = crypto$1.createHash("sha256").update(code_verifier).digest();
...
async function GetintegrationSettingsData(services, req, getSchema) {
  ...
  try {
    ...
    const integrationSettingsData = integrationSettings[0];
    if (!integrationSettingsData.facebook_app_id || !integrationSettingsData.facebook_app_secret || !integrationSettingsData.public_directus_url || !integrationSettingsData.webhook_verify_token) {
...
async function GetIntegrationLogsService(services, req, getSchema) {
  try {
    const { ItemsService } = services;
    const schema = await getSchema();
...
async function GetOmnichannelsService(services, req, getSchema) {
  try {
    const { ItemsService } = services;
    const schema = await getSchema();
...
async function AddOrUpdateFBOmnichannel(OmnichannelsService, page, isEnabled = false) {
  try {
    console.log(
...
async function UpdateFBOmnichannel(directusPageId, OmnichannelsService, page, isEnabled = false) {
  try {
    const updateOmnichannel = {
      page_name: page.name,
...
async function AddNewFBOmnichannel(OmnichannelsService, page, isEnabled) {
  try {
    const newOmichannel = {
      page_id: page.id,
      page_name: page.name,
...
async function AddOrUpdateIGOmnichannel(OmnichannelsService, user, userAccessToken, expiresIn, isEnabled = false) {
  try {
    console.log(
      `Adding or updating omini channel for user: ${user.username} (${user.user_id})`
    );
    const currentDate = /* @__PURE__ */ new Date();
    const millisecondsToAdd = expiresIn * 1e3;
...
async function UpdateIGOmnichannel(directusPageId, OmnichannelsService, user, userAccessToken, expiredDate, isEnabled = false) {
  try {
    const updateOmnichannel = {
...
async function AddNewIGOmnichannel(OmnichannelsService, user, userAccessToken, expiredDate, isEnabled) {
  try {
    const newOmichannel = {
...
async function AddOrUpdateZaloOAOmnichannel(OmnichannelsService, page, accessToken, refreshToken, expiresIn, isEnabled = false) {
  try {
    const currentDate = /* @__PURE__ */ new Date();
    const millisecondsToAdd = expiresIn * 1e3;
    const newTimestamp = currentDate.getTime() + millisecondsToAdd;
...
async function UpdateZaloOAOmnichannel(directusPageId, OmnichannelsService, page, accessToken, refreshToken, expiredDate, isEnabled = false) {
  try {
    const updateOmnichannel = {
      page_name: page.name,
...
async function AddZaloOANewOmnichannel(OmnichannelsService, page, accessToken, refreshToken, expiredDate) {
  try {
    const newOmichannel = {
      page_id: page.oa_id,
      page_name: page.name,
...
async function LogIntegrationEvent(services, req, getSchema, logEntry) {
  try {
...
async function LogInformationEvent(req, services, getSchema, message, stack_trace = "", context = "") {
  try {
...
const textEncoder = typeof TextEncoder === 'function' ? new TextEncoder() : new util$4.TextEncoder();
  ...
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    if (utils$1.isSpecCompliantForm(data)) {
      ...
      if (!headers.hasContentLength()) {
        try {
          const knownLength = await util$4.promisify(data.getLength).call(data);
...
axiosInstance.interceptors.response.use(
  ...
  (error) => Promise.reject(
    error.response && error.response.data && error.response.data.error || "Something went wrong!"
  )
);
const IGAxiosInstance = axios$1.create();
...
async function GetFBShortLiveToken(integrationSettingsData, redirectUri, code) {
  try {
    const tokenExchangeUrl = `/oauth/access_token?client_id=${integrationSettingsData.facebook_app_id}&client_secret=${integrationSettingsData.facebook_app_secret}&redirect_uri=${encodeURIComponent(redirectUri)}&code=${code}`;
...
async function GetFBLongLiveToken(integrationSettingsData, shortLivedUserAccessToken) {
  try {
    const longLivedTokenExchangeUrl = `/oauth/access_token?grant_type=fb_exchange_token&client_id=${integrationSettingsData.facebook_app_id}&client_secret=${integrationSettingsData.facebook_app_secret}&fb_exchange_token=${shortLivedUserAccessToken}`;
...
async function SubscribeFacebookPageWebhook(pageId, pageAccessToken) {
...
async function GetAuthenticatedUserPages(userAccessToken) {
  try {
...
async function SubscribePagesWebhook(req, services, getSchema, pages) {
...
async function ConfigureWebhook(integrationSettingsData) {
  try {
...
async function GetAppAccessToken(integrationSettingsData) {
  try {
    const appAccessTokenUrl = `/oauth/access_token?client_id=${integrationSettingsData.facebook_app_id}&client_secret=${integrationSettingsData.facebook_app_secret}&grant_type=client_credentials`;
...
async function GetIGShortLiveToken(integrationSettingsData, redirectUri, code) {
  try {
...
async function GetIGLongLiveToken(integrationSettingsData, shortLivedUserAccessToken) {
  try {
    const longLivedTokenExchangeUrl = `https://graph.instagram.com/access_token?grant_type=ig_exchange_token&client_secret=${integrationSettingsData.instagram_app_secret}&access_token=${shortLivedUserAccessToken}`;
    ...
    return {
      accessToken: longLivedTokenResponse.data.access_token || shortLivedUserAccessToken,
...
async function SubscribeIGWebhook(userId, userAccessToken) {
  const subscribeUrl = `https://graph.instagram.com/v23.0/${userId}/subscribed_apps?subscribed_fields=messages&access_token=${userAccessToken}`;
  const fieldsToSubscribe = "messages";
  try {
...
async function GetIGAuthenticatedUser(userAccessToken) {
  try {
    const url = `https://graph.instagram.com/v23.0/me?access_token=${userAccessToken}&fields=user_id,account_type,name,profile_picture_url,username`;
...
async function SubscribeIGAccountWebhook(req, services, getSchema, user, userAccessToken, expiresIn) {
...
async function GetZaloAccessToken(integrationSettingsData, code, codeVerifier) {
  try {
    ...
    const tokenResponse = await axios$1.post(
...
async function GetAuthenticatedZaloOAPage(accessToken) {
  try {
    const pagesUrl = `https://openapi.zalo.me/v2.0/oa/getoa`;
    const pagesResponse = await axios$1.get(pagesUrl, {
      headers: {
        access_token: accessToken
      }
    });
    const page = pagesResponse.data.data;
    if (!page) {
...
async function handleFacebookCallback(req, res, services, getSchema) {
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.stack || error.message : typeof error === "string" ? error : JSON.stringify(error);
    const logId = await LogIntegrationEvent(services, req, getSchema, {
      level: "error",
...
async function handleInstagramCallback(req, res, services, getSchema) {
  const code = req.query.code;
  const integrationSettingsData = await GetintegrationSettingsData(
    services,
    req,
...
async function handleZaloOACallback(req, res, services, getSchema) {
  const code = req.query.code;
...
async function handleFacebookAuthRequest(req, res, services, getSchema) {
  const integrationSettingsData = await GetintegrationSettingsData(
    services,
    req,
    getSchema
  ...
  try {
    const redirectUri = `${integrationSettingsData.public_directus_url}/directus-extension-social-connector/api/facebook/auth/callback`;
    const facebookAuthUrl = `https://www.facebook.com/v23.0/dialog/oauth?client_id=${integrationSettingsData.facebook_app_id}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=${integrationSettingsData.scopes.join(",")}&response_type=code`;
...
async function handleInstagramAuthRequest(req, res, services, getSchema) {
  const integrationSettingsData = await GetintegrationSettingsData(
    services,
    req,
    getSchema
  ...
  try {
    const redirectUri = `${integrationSettingsData.public_directus_url}/directus-extension-social-connector/api/instagram/auth/callback`;
    const instagramAuthUrl = `https://www.instagram.com/oauth/authorize?client_id=${integrationSettingsData.instagram_app_id}&redirect_uri=${redirectUri}&scope=${integrationSettingsData.instagram_scopes.join(",")}&response_type=code`;
...
async function handleZaloOAAuthRequest(req, res, services, getSchema) {
  ...
  try {
    ...
    const zaloAuthUrl = `https://oauth.zaloapp.com/v4/oa/permission?app_id=${integrationSettingsData.zalo_app_id}&redirect_uri=${encodeURIComponent(redirectUri)}&code_challenge=${code_challenge}`;
...
var index = defineEndpoint((router, { services, getSchema, env }) => {
  router.get(
    "/api/facebook/auth",
    (req, res) => handleFacebookAuthRequest(req, res, services, getSchema)
  );
  router.get(
    "/api/facebook/auth/callback",
    (req, res) => handleFacebookCallback(req, res, services, getSchema)
  );
  router.get(
    "/api/instagram/auth",
    (req, res) => handleInstagramAuthRequest(req, res, services, getSchema)
  );
  router.get(
    "/api/instagram/auth/callback",
    (req, res) => handleInstagramCallback(req, res, services, getSchema)
  );
  router.get(
    "/api/zalo/auth",
    (req, res) => handleZaloOAAuthRequest(req, res, services, getSchema)
  );
  router.get(
    "/api/zalo/auth/callback",
    (req, res) => handleZaloOACallback(req, res, services, getSchema)
  );
});
